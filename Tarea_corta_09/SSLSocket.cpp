...
SSLSocket* SSLSocket::AcceptConnection() {
    struct sockaddr_in6 client_addr6;  // estructura para IPv6
    socklen_t client_size = sizeof(client_addr6);

    int new_fd = accept(this->idSocket, (struct sockaddr*)&client_addr6, &client_size);
    if (new_fd < 0) {
        perror("Error accepting IPv6 connection");
        return nullptr;
    }
