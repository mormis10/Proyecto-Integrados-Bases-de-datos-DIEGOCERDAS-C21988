...
void *start_servers_connection(void *data) {
  ...
  if (bind(sock_udp, (struct sockaddr *)&server_addr, sizeof(server_addr)) <
      0) {
  ...
  char buffer_udp[201];
  ssize_t st = recvfrom(sock_udp, buffer_udp, 200, 0, (struct sockaddr *)&other,
                        &addr_len);
  if (st < 0) {
  ...
  std::string result =
      std::string(ip) + std::string(" ") + std::string(figureN);
  const char *message = result.c_str();
  ssize_t sent_bytes =
      sendto(sock_udp_send, message, strlen(message), 0,
             (const struct sockaddr *)&udp_mess, sizeof(udp_mess));
  ...
  if (bind(sockid, (struct sockaddr *)&server_addr_tcp,
  ...
  pthread_mutex_lock(tdata->shared_mutex);
  std::cout << "El hilo número: " << tdata->island_id
            << " ya está escuchando\n";
  pthread_mutex_unlock(tdata->shared_mutex);
  ...
  //int bytes_read = recvfrom(client_id, buffer, size, 0, (struct sockaddr
*)addr, &addr_len); buffer[bytes_read] = '\0';
pthread_mutex_lock(tdata->shared_mutex);
